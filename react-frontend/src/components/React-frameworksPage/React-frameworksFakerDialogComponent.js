...
import react-frameworksFakerFactory from "./react-frameworksFakerFactory";
...
const React-frameworksFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = react-frameworksFakerFactory(fakerCount);
        let results = [];
        setLoading(true);
        try {
            for (let i = 0; i < fakeData.length; i++) {
                const _data = fakeData[i];
                const res = await client.service("react-frameworks").create(_data);
                // const res = _data;
                results = [...results, res];
            }
            props.onHide();
...
export default connect(null, mapDispatch)(React-frameworksFakerDialogComponent);

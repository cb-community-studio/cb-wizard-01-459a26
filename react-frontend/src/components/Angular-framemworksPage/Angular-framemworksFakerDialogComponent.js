...
import angular-framemworksFakerFactory from "./angular-framemworksFakerFactory";
...
const Angular-framemworksFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = angular-framemworksFakerFactory(fakerCount);
        let results = [];
        setLoading(true);
        try {
            for (let i = 0; i < fakeData.length; i++) {
                const _data = fakeData[i];
                const res = await client.service("angular-framemworks").create(_data);
                // const res = _data;
                results = [...results, res];
            }
            props.onHide();
...
export default connect(null, mapDispatch)(Angular-framemworksFakerDialogComponent);

...
import svelte-frameworksFakerFactory from "./svelte-frameworksFakerFactory";
...
const Svelte-frameworksFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = svelte-frameworksFakerFactory(fakerCount);
        let results = [];
        setLoading(true);
        try {
            for (let i = 0; i < fakeData.length; i++) {
                const _data = fakeData[i];
                const res = await client.service("svelte-frameworks").create(_data);
                // const res = _data;
                results = [...results, res];
            }
            props.onHide();
...
export default connect(null, mapDispatch)(Svelte-frameworksFakerDialogComponent);

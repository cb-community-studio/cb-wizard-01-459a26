...
import frontend-jsFakerFactory from "./frontend-jsFakerFactory";
...
const Frontend-jsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = frontend-jsFakerFactory(fakerCount);
        let results = [];
        setLoading(true);
        try {
            for (let i = 0; i < fakeData.length; i++) {
                const _data = fakeData[i];
                const res = await client.service("frontend-js").create(_data);
                // const res = _data;
                results = [...results, res];
            }
            props.onHide();
...
export default connect(null, mapDispatch)(Frontend-jsFakerDialogComponent);

...
import vue-frameworksFakerFactory from "./vue-frameworksFakerFactory";
...
const Vue-frameworksFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = vue-frameworksFakerFactory(fakerCount);
        let results = [];
        setLoading(true);
        try {
            for (let i = 0; i < fakeData.length; i++) {
                const _data = fakeData[i];
                const res = await client.service("vue-frameworks").create(_data);
                // const res = _data;
                results = [...results, res];
            }
            props.onHide();
...
export default connect(null, mapDispatch)(Vue-frameworksFakerDialogComponent);

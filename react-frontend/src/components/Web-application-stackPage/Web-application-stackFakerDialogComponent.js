...
import web-application-stackFakerFactory from "./web-application-stackFakerFactory";
...
const Web-application-stackFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = web-application-stackFakerFactory(fakerCount);
        let results = [];
        setLoading(true);
        try {
            for (let i = 0; i < fakeData.length; i++) {
                const _data = fakeData[i];
                const res = await client.service("web-application-stack").create(_data);
                // const res = _data;
                results = [...results, res];
            }
            props.onHide();
...
export default connect(null, mapDispatch)(Web-application-stackFakerDialogComponent);
